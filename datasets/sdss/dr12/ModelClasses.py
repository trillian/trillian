...
metadata_pickle_filename = "ModelClasses_photodb.pickle"
...
class Run(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.run']
	else:
		__tablename__ = 'run'
		__table_args__ = {'autoload':True, 'schema':'dataset_sdss_dr12'}

	def __repr__(self):
		return '<Run (pk={0}, run={1})>'.format(self.pk, self.run.run)

class Field(Base):
	
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.field']
	else:
		__tablename__ = 'field'
		__table_args__ = {'autoload':True, 'schema':'dataset_sdss_dr12'}
...
class FlagType(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.flag_type']
	else:
...
class FlagValue(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.flag_value']
	else:
...
class FieldToFlag(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.field_to_flag']
	else:
...
class PhotoObjToFlag(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.photoobj_to_flag']
	else:
...
class ObjectType(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.object_type']
	else:
...
class Thing(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.thing']
	else:
		__tablename__ = 'thing'
		__table_args__ = {'autoload':True, 'schema':'dataset_sdss_dr12'}
...
class PhotoObjToThing(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.photoobj_to_thing']
	else:
...
class PhotoObj(Base):
	...
	if cached_metadata:
		__table__ = cached_metadata.tables['dataset_sdss_dr12.photoobj']
	else:
	...
	#for col in inspector.get_columns('photoobj', schema='dataset_sdss_dr12'):
	#	if col["name"] == "pk":
	#		break
...
if not cached_metadata:
	...
	try:
		...
		with open(os.path.join(cache_path, metadata_pickle_filename), 'w') as cachefile:
			pickle.dump(Base.metadata, cachefile)
	except:
